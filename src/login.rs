        ...
        match msg {
            LoginMsg::ValidateExistEmail(operation) => {
                ...
                ctx.link().send_future(async move {
                    match common::validate_exist_email(&email).await {
                        Ok(_) => LoginMsg::ValidateExistEmailSuccess(operation),
                        Err(err) => LoginMsg::ValidateExistEmailFail(Box::new(err)),
                    }
                });
                false
            }
            LoginMsg::ValidateExistEmailSuccess(operation) => {
                ...
                match operation {
                    ...
                    ValidateExistEmailOperation::Login => {
                        ...
                        ctx.link().send_future(async move {
                            match request::post::<String, _>(request::Host::ApiBase, "/login", &req)
                                .await
                            {
                                Ok(res) => LoginMsg::HandleLoginSuccess {
                                    token: res.data.unwrap(),
                                },
                                Err(err) => LoginMsg::HandleLoginFail(Box::new(err)),
                            }
                        });
                    }
                }
            ...
            LoginMsg::PasswordChange(e) => {
                ...
                match common::validate_pwd(&self.req.pwd) {
            ...
            LoginMsg::HandleLoginSuccess { token } => {
                ...
                ctx.link().send_future(async move {
                    match request::get::<common::CurrentUser, Vec<(&str, &str)>, _>(
                    ...
                    .await
                    {
                        Ok(res) => LoginMsg::HandleCheckTokenSuccess(res.data.unwrap()),
                        Err(err) => LoginMsg::HandleLoginFail(Box::new(err)),
                    }
                });
                false
            ...
            LoginMsg::HandleLoginFail(e) => {
                self.request_fail_msg = format!("login failed: {}", e);
                true
            }
            ...
            <section class="hero is-fullheight">
                ...
                <div class="login">
                    ...
                    <div class="field">
                        ...
                        <input ref={&self.refs[0]} class={email_input_class} value={self.req.email.clone()} type="email" placeholder="hello@example.com" onkeyup={ctx.link().callback(|e:web_sys::KeyboardEvent|LoginMsg::EmailChange(e))} onkeydown={ctx.link().callback(|e:web_sys::KeyboardEvent|LoginMsg::KeyDownLogin(e))} />
                    ...
                    <div class="field">
                        <div class="control has-icons-left">
                        <input class={password_input_class} value={self.req.pwd.clone()} type="password" placeholder="**********" onkeyup={ctx.link().callback(|e:web_sys::KeyboardEvent|LoginMsg::PasswordChange(e))} onkeydown={ctx.link().callback(|e:web_sys::KeyboardEvent|LoginMsg::KeyDownLogin(e))}/>
                            <span class="icon is-small is-left">
